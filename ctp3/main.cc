...
void usage(ostream &os) {
    os << "usage: ctp3 [--no-colors] [{-r | --request} <request>]* [{-s | --seed} <default-seed>] [{-t | --trials} <num-trials>] [{-c | --shortcut-cost} <cost>] [{-d | --dead-end-value} <value>] [{-f | --calculate-features} <nsamples>] <file>" << endl;
}
