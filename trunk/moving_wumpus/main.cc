...
int main(int argc, const char **argv) {
    ...
    if( (h == 1) || (h = 11) ) {
        heuristic = new shortest_distance_to_unvisited_cell_t<state_t>(problem, compass);
        if( h == 11 ) {
    ...
    Heuristic::heuristic_t<m_state_t> *m_heuristic = new shortest_distance_to_unvisited_cell_t<m_state_t>(m_problem, compass);
    ...
    wumpus_base_policy_t<state_t> wumpus(problem, nrows, ncols, compass);
